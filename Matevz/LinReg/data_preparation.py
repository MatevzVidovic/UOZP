...
def sparse_std(X, means):
    N = X.shape[0]
    sqr = X.copy()
    sqr -= means
    sqr = np.square(sqr)
    variance = np.sum(sqr, axis=0)/N
...
class MyStandardizer:
    ...
    def fit(self, X):
        ...
        self.std_devs = sparse_std(data, self.means)
    ...
    def transform(self, X):
        ...
        #     return
        data = X.copy()
        returner = data - self.means
        returner = returner / self.std_devs
        returner = csr_matrix(returner)
        return returner
