...
PARAMS = {
    "col_norm" : "standardize", #"standardize", "dont" or "L2"
}
...
class Model:
    ...
    def predict(self, test_cases_list, all_together_model=False):
        ...
        for topic, DT in topic_2_train_DT.items():
            ...
            y_pred_final[original_ixs] = np.reshape(y_pred, (-1))


        return X_test_final, y_test_final, y_pred_final, None
...
def test_model(model, test_cases_list, all_topics_together=False):
    ...
    if all_topics_together:
        ...
        X_test, y_test, y_pred, _ = model.predict(test_cases_list)
    ...
    if all_topics_together:
        ...
        try:
            ...
            sorted_coefficients_ixs = sorted_coefficients_ixs[:-15:-1]
            best_coeffs_names =[attr_names[ix] for ix in sorted_coefficients_ixs]
            best_coefs_vals = [coefficients[ix]/10 for ix in sorted_coefficients_ixs]
            ...
            sorted_coefficients_ixs = np.argsort(np.abs(coefficients))
            sorted_coefficients_ixs = sorted_coefficients_ixs[-15:-30:-1]
            best_coeffs_names =[attr_names[ix] for ix in sorted_coefficients_ixs]
            best_coefs_vals = [coefficients[ix]/10 for ix in sorted_coefficients_ixs]
            ...
            print("Couldn't plot the coefficients.")
            
    return returner

def run(train_data, test_data, hyper_parameters=None, all_topics_together=False):
    ...
    if all_topics_together == "both":
        scores_1 = test_model(curr_model, test_data, all_topics_together=True)
        scores_2 = test_model(curr_model, test_data, all_topics_together=False)
        returner = [scores_1, scores_2]
    else:
        returner = test_model(curr_model, test_data, all_topics_together=all_topics_together)
